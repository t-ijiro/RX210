...
{
    g->is_reset         = (option >> 0) & 1;
    g->is_buzzer_active = (option >> 1) & 1;
    g->is_man_vs_man    = (option >> 2) & 1;
	g->is_man_vs_AI     = (option >> 3) & 1;
    g->is_AI_vs_AI      = (option >> 4) & 1;
	g->is_AI_turn       = (option >> 5) & 1;
	g->is_skip          = (option >> 6) & 1;
}
